/**
 * simulator.js
 * Fires production-accurate STK callbacks at a local server.
 * No ngrok. No HTTPS. No internet. Just HTTP POST to localhost.
 */
...
const DEFAULT_DELAY_MS = 2000; // simulate real ~2s Safaricom delay

/**
 * Fire a single callback payload at the target URL.
...
export async function simulate(opts) {
  ...
  // Duplicate scenario returns array of two payloads
  ...
  for (let i = 0; i < payloads.length; i++) {
    ...
    // Persist to replay log before firing
    if (!noQueue) {
